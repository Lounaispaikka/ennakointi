...
?> <? /*
    <div id="news-5">
        ...
        <div id="news-archive">
                <div id="news-archive_left" style="display: block; float: left; <? if(count($Tapahtumat) != null) { ?>width:562px; <? } ?>">
                    <h2>Uutiset</h2>
                    <ul class="content">               
                        ...
                        <?	}
							if($uutisia_kpl==10) { ?> 
								<a href="#" id="show_all_uutiset">Lis&auml;&auml; uutisia...</a>
							<?}
               ...
               <? if(count($Tapahtumat) != null) { ?>
                <div id="news_tapahtumat" style="width: 320px; display: block; float:right; border-width: 0px;">
						...
						?>
                            <li id="lnid<?=$Tapahtuma->id?>" name="n<?=$Tapahtuma->id?>" class="pag">
                               <div id="nhead">
						...
						<?	
							if($tapahtumia_kpl==10) { ?> 
								<a href="#" id="show_all_tapahtumat">Lis&auml;&auml; uutisia...</a>
							<?}
							if($tapahtumia_kpl>10) {	?>
							<li id="lnid<?=$Tapahtuma->id?>" name="n<?=$Tapahtuma->id?>" class="pag" style="display: none;">
                               <div id="nhead">
                                        <a name="n<?=$Tapahtuma->id?>"></a>
                                        <a href="<?=$Page->getPageUrl()?>?nid=<?=$Tapahtuma->id?>#n<?=$Tapahtuma->id?>" class="nid" data-nid="<?=$Tapahtuma->id?>">
                                        <h1><?=$Tapahtuma->title?></h1>
...
$(function(){
   ...
   //N‰yt‰ kaikki uutiset
	...
	$(".pag").show();$("#show_all_uutiset").hide(); //n‰ytt‰‰ kaikki uutiset, jotta linkit ajankohtaisiin toimisivat
