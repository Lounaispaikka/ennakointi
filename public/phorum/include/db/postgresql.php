...
/***
 * Close the connection to the Postgresl server
 */
...
function phorum_db_post_message(&$message, $convert=FALSE)
{
	...
	if (!isset($insertfields['message_id'])) {
		...
		$message_id=$insertfields['message_id'];
	}
        ...
        else
        {
            if (count($tokens))
            {
				foreach ($tokens as $k=>$v)
					$tokens[$k]=phorum_db_interact(DB_RETURN_QUOTED,$v);

                $condition = ($match_type == "ALL") ? "AND" : "OR";

                $match_str = "'%".implode("%' $condition body ilike '%", $tokens)."%'";
            }
...
function phorum_db_user_get_moderators($forum_id, $exclude_admin=FALSE, $for_email=FALSE)
{
    ...
    $usermods = phorum_db_interact(
        DB_RETURN_ROWS,
        "SELECT DISTINCT tuser.user_id AS user_id,
                tuser.email AS email
         FROM   {$PHORUM['user_table']} AS tuser
                LEFT JOIN {$PHORUM['user_permissions_table']} AS perm
                ON perm.user_id = tuser.user_id
    ...
    $groupmods = phorum_db_interact(
        ...
        "SELECT DISTINCT tuser.user_id AS user_id,
         ...
         WHERE  tuser.user_id       = usergroup.user_id AND
...
function phorum_db_newflag_update_forum($message_ids)
{
    ...
    phorum_db_interact(
        DB_RETURN_RES,
			"UPDATE {$GLOBALS['PHORUM']['user_newflags_table']} AS flags
				SET    forum_id = msg.forum_id
				FROM   {$GLOBALS['PHORUM']['message_table']} AS msg
				WHERE  flags.message_id = msg.message_id AND flags.message_id IN ($ids_str)",
        NULL,
        DB_MASTERQUERY
    );
}
